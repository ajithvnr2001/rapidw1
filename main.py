...
def run_autopdf(incident_id: int, update_solution: bool = False) -> Dict:
    ...
    glpi_client = GLPIClient()  # Keep GLPIClient instantiation for now, but it won't be used by DataExtractorAgent
            ...
            if event["itemtype"] == "Ticket":
                ...
                if event["event"] in ("add", "update"):
                    ...
                    if event["event"] == "update":
                        result = run_autopdf(incident_id, update_solution=True)  # Pass the incident ID and update flag.
                    else:
                        result = run_autopdf(incident_id)  # Don't update solution on add

                    results.append(result)  # Append individual results.
